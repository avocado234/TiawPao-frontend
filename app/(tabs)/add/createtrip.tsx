...
export default function CreateTrip() {
  ...
  // Animation values สำหรับ dropdown
  ...
  const toggleRegionDropdown = () => {
    if (regionHeight.value > 0) {
      ...
      // ปิด province dropdown เมื่อเปิด region
  ...
  const toggleProvinceDropdown = () => {
    ...
    if (provinceHeight.value > 0) {
      ...
      // ปิด region dropdown เมื่อเปิด province
      regionHeight.value = withTiming(0, { duration: 300 });
      regionChevronRotation.value = withSpring(0);
    }
  };
  ...
  // Handlers สำหรับการเปลี่ยนแปลง Date/Time
  ...
  // useFocusEffect เพื่อรีเซ็ต state ทุกครั้งที่หน้ามี focus
  useFocusEffect(
    useCallback(() => {
      setTripName('');
      setSelectedValueRegion(null);
      setSelectedValueProvince(null);
      setSelectedValueProvinceID(null);
